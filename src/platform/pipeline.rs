...
use crate::{
    show_editor_ui, show_level_select_ui, show_menu_wordmark_ui, show_splash_screen_ui, State,
};
    ...
    pub fn run_frame(&mut self, state: &mut State, raw_input: egui::RawInput) -> egui::FullOutput {
        ...
        let full_output = self.egui_ctx.run(raw_input, |ctx| {
            ...
            if let Some(wordmark) = &self.menu_wordmark {
                ...
                let commands = show_menu_wordmark_ui(ctx, state, wordmark);
                menu_commands.extend(commands);
            }
            // Show level select UI when in level select mode
            let commands = show_level_select_ui(ctx, state);
            menu_commands.extend(commands);
        });

        // Dispatch any commands generated by the UI
