...
impl State {
    ...
    fn record_editor_history_state(&mut self) {
        if self.phase != AppPhase::Editor {
            return;
        }
    ...
    fn editor_paste_block(&mut self) {
        if self.phase != AppPhase::Editor {
            return;
        }
    ...
    fn topmost_block_index_at_cursor(&self, cursor: [i32; 3]) -> Option<usize> {
